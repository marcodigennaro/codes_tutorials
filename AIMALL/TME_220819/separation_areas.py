...
for i in range(len(ION_SYSTEM)):
    ...
    SUMFILE_PATH = '/home/mdi0316/WORK/AUG/AIMALL/TME_220819/EMIM-BF4/T90/P90/N311/B3LYP/sum'
    ...
    files = get_infiles_in_order(input_path=DATAPATH, reverse=False)
    d_CM = []
    separation_area_between_atoms = []
    separation_area_between_ions = []
    for file in files:
        with open(DATAPATH + file, 'r') as f:
            index = parse_index_out_of_filename(file)
            dist = int(index) / 100.
            d_CM.append(dist)
